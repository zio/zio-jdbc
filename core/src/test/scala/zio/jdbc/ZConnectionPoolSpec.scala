  ...
  def spec: Spec[TestEnvironment, Any] =
    suite("ZConnectionPoolSpec") {
      def testPool(config: ZConnectionPoolConfig = ZConnectionPoolConfig.default) = for {
        ...
        getConn = ZIO.succeed(new TestConnectionValid).tap(conns.offer(_))
        ...
        test("auto invalidate dead connections") {
          def testPoolInvalid(
            ...
            for { //Pool that contains some invalid connections
              conns  <- Queue.unbounded[TestConnectionInvalid]
              getConn = ZIO.succeed(new TestConnectionInvalid).tap(conns.offer(_))
              pool   <- ZLayer.succeed(config).to(ZConnectionPool.make(getConn)).build.map(_.get)
            } yield conns -> pool
          ZIO.scoped {
            for {
...
class TestConnectionValid extends ZConnectionPoolSpec.TestConnection {
  ...
  override def prepareStatement(sql: String): PreparedStatement = new DummyPreparedStatement()

}

class TestConnectionInvalid extends ZConnectionPoolSpec.TestConnection {
