...
sealed trait SqlFragment { self =>
   ...
   */
  def insert(returnAutoGeneratedKeys: Boolean = true): ZIO[ZConnection, Throwable, UpdateResult] =
    ZIO.scoped(executeWithUpdateResult(self, returnAutoGeneratedKeys))
  ...
  private def executeWithUpdateResult(sql: SqlFragment, returnAutoGeneratedKeys: Boolean): ZIO[Scope with ZConnection, Throwable, UpdateResult] =
    for {
      connection <- ZIO.service[ZConnection]
      result     <- connection.executeSqlWith(sql, returnAutoGeneratedKeys) { ps =>
                      for {
                        ...
                        (count, rs) = result
                        keys       <- ZIO.fromOption(rs).flatMap(rs => ZIO.attempt {
